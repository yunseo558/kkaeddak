...
현재 구현 범위는 프로젝트 기반, OpenAPI 계약, 데이터베이스 계층과 데모 세션·프로필·루틴·정규화 일정·준비 작업·기상 계획·집계 결과·추천 이유·날짜별 설명 가능한 기상 리포트 API와 운영 헬스체크입니다.
...
날짜별 리포트는 기상 계획을 만든 당시의 일정, 수면 분, 활동·컨디션 등급,
최근 기상 집계, AI 피로도·설명·신뢰도와 정책 버전을 스냅샷으로 보존합니다.
원본 HealthKit 샘플은 받지 않습니다. 각 알람 단계의 울림·해제·기상 확인·미확인
이벤트와 학습 전후 보정값을 기록하면 `GET /api/v1/history/reports/{localDate}`에서
당일 판단 근거, 실행 타임라인, 결과와 다음 계획에 미친 영향을 함께 조회할 수 있습니다.
...
커밋하지 않습니다. 모델 기본값은 `gemini-3.5-flash-lite`입니다.

- `KKAEDDAK_GEMINI_API_KEY`
- `KKAEDDAK_GEMINI_MODEL` (선택)
- `KKAEDDAK_GEMINI_TIMEOUT_SECONDS` (선택, 기본 10초)

개인화는 서비스의 핵심 기능으로 항상 실행됩니다. 일정 분류에는 일정 이름과
사용자가 정한 유형 이름만 사용하고, 일정 메모·장소는 전송하지 않습니다.
기상 개인화에는 수면 분, 활동·컨디션 등급, 최근 기상
성공·지각·미확인 횟수 등 요약값만 보냅니다. Gemini는 피로도, 첫 알람
시작 시각, 알람 개수·간격과 판단 근거를 JSON Schema 구조화 출력으로
반환합니다. 서버와 클라이언트는 최대 4개·90분 범위, 학습된 안전
하한, 14일 승인 정책을 AI 출력과 별도로 검증합니다.
